...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        THRESHOLD = 0.8
        # Get all watched & unwatched movies
        ...
        for unwatched_movie in unwatched_movies:
            ...
            for watched_movie in watched_movies:
                ...
                # Stop early if the unwatched_movie is similar enough
                if max_similarity >= THRESHOLD:
                    break
            # If unwatched_movie is similar enough to watched movies, then recommend it
            if max_similarity > THRESHOLD:
                will_recommend = True
                print(f"Found a new movie recommendation: {unwatched_movie.original_title}")
